...
export function useAIFlowyModal<TParentModalProps extends ModalProps = ModalProps>(
...
async function checkProps(api: ExtendedModalApi, attrs: Record<string, any>) {
  ...
  for (const attr of Object.keys(attrs)) {
    if (stateKeys.has(attr) && !['class'].includes(attr)) {
      ...
      console.warn(
        `[AIFlowy Modal]: When 'connectedComponent' exists, do not set props or slots '${attr}', which will increase complexity. If you need to modify the props of Modal, please use useAIFlowyModal or api.`,
      );
    }
  }
}
