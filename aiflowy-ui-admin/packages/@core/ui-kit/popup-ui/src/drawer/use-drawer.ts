...
export function useAIFlowyDrawer<
...
>(options: DrawerApiOptions = {}) {
  ...
  const Drawer = defineComponent(
    (props: DrawerProps, { attrs, slots }) => {
      return () =>
        h(AIFlowyDrawer, { ...props, ...attrs, drawerApi: extendedApi }, slots);
    },
    // eslint-disable-next-line vue/one-component-per-file
    {
      name: 'AIFlowyDrawer',
...
async function checkProps(api: ExtendedDrawerApi, attrs: Record<string, any>) {
  ...
  for (const attr of Object.keys(attrs)) {
    if (stateKeys.has(attr) && !['class'].includes(attr)) {
      // connectedComponent存在时，不要传入Drawer的props，会造成复杂度提升，如果你需要修改Drawer的props，请使用 useAIFlowyDrawer 或者api
      console.warn(
        `[AIFlowy Drawer]: When 'connectedComponent' exists, do not set props or slots '${attr}', which will increase complexity. If you need to modify the props of Drawer, please use useAIFlowyDrawer or api.`,
      );
    }
  }
}
